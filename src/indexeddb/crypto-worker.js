...
async function encryptBlock(name, data) {
  let key = dbKeys.get(name);
  if (key) {
...
async function decryptBlock(name, data) {
  let key = dbKeys.get(name);
  if (key) {
    data = await decrypt(key, data);
  }
  return data;
}
...
async function listen(reader, writer) {
  ...
  switch (method) {
    ...
    case 'loadKey': {
      ...
      let keyData = reader.bytes();
      reader.done();

 console.log('%%%% loadKey', crc32(keyData), keyData);
      let key = await self.crypto.subtle.importKey('raw', keyData, 'AES-GCM', false, ['encrypt', 'decrypt']);
      dbKeys.set(name, key);
    ...
    case 'encryptBlock': {
      let name = reader.string();
      let num = reader.int32();
      let plaintext = reader.bytes();
      reader.done();

// console.log('%%%% encryptBlock 1', num, crc32(plaintext), plaintext);
      let ciphertext = await encryptBlock(name, plaintext);
// console.log('%%%% encryptBlock 2', num, crc32(ciphertext), ciphertext);
...
// console.log('%%%% decryptBlock 1', num, crc32(ciphertext), ciphertext);
      let plaintext = await decryptBlock(name, ciphertext);
// console.log('%%%% decryptBlock 2', num, crc32(plaintext), plaintext);
